...
import { Cartesian3,Transforms,Rectangle, defaultValue, when, Check, Resource, ShadowMode } from 'cesium'
    ...
    S3MTilesLayer.prototype.loadConfig = function (url) {
      ...
      when(url)
        ...
        .then(function (config) {
          ...
          that._position = Cartesian3.fromDegrees(lon, lat, height);
          that.modelMatrix = Transforms.eastNorthUpToFixedFrame(that._position);
          that._rectangle = Rectangle.fromDegrees(config.geoBounds.left, config.geoBounds.bottom, config.geoBounds.right, config.geoBounds.top);
